...
class Main(tk.Frame): 
    ...
    def init_main(self): 
        ...
        self.tree.column("salary", width=100, anchor=tk.CENTER) #Параметры колонки зарплата

        self.tree.heading("ID", text="ID") 
        self.tree.heading("name", text="ФИО")
        self.tree.heading("tel", text="Телефон")
        self.tree.heading("email", text="E-mail")
        self.tree.heading("salary", text='Зарплата') #Создание удобочитаемого вида зарплата
        ...
        btn_refresh = tk.Button(
            toolbar, bg="#d7d8e0", bd=0, image=self.refresh_img, command=self.view_records  ) 
    ...
    def update_records(self, name, tel, email, salary):
        selected_item_id = self.tree.set(self.tree.selection()[0], "#1")
        self.db.cursor.execute(
            """UPDATE db SET name=?, tel=?, email=?, salary=? WHERE id=?""",
            (name, tel, email, salary, selected_item_id),
            )
        self.db.conn.commit()
        self.view_records()
    ...
    def delete_records(self):
        for selection_items in self.tree.selection():
            selected_item_id = self.tree.set(selection_items, "#1")
            self.db.cursor.execute(
                "DELETE FROM db WHERE id=?", (selected_item_id,)
            )
        self.db.conn.commit()
        self.view_records()
...
class Child(tk.Toplevel):
    ...
    def init_child(self): 
        ...
        label_salary = tk.Label(self, text="Зарплата:") #Поле для зарплаты
        ...
        self.entry_salary = ttk.Entry(self) #Создание поля для ввода зарплаты
